...
from models.parking import ParkingSlot, ParkingZone, SlotStatus
from models.user import User, UserRole
from schemas.booking import BookingCreate, BookingRead, BookingUpdateStatus
...
async def confirm_booking(
    ...
    # Backward-compatible: treat "confirm" as arrival confirmation.
    booking.status = BookingStatus.active
    booking.arrival_confirmed_at = datetime.now(timezone.utc)
    booking.arrival_confirmed_by = "user" if booking.user_id == current_user.id else "admin"
    slot = db.scalar(select(ParkingSlot).where(ParkingSlot.id == booking.slot_id).with_for_update())
    if slot is not None:
...
@router.post("/{booking_id}/arrive", response_model=BookingRead)
async def arrive_booking(
    ...
    db.commit()
    db.refresh(booking)
    await broadcast_event("booking.arrived", {"booking_id": str(booking.id), "status": booking.status.value})
    return _serialize_booking(booking)
...
async def depart_booking(
    ...
    db.commit()
    db.refresh(booking)
    await broadcast_event("booking.departed", {"booking_id": str(booking.id), "status": booking.status.value})
    return _serialize_booking(booking)


@router.post("/{booking_id}/claim-hold", response_model=BookingRead)
async def claim_hold(
    booking_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> BookingRead:
    ...
    if booking.hold_until is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No active hold for this booking")
    if booking.hold_until <= datetime.now(timezone.utc):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Hold has expired")

    slot = db.scalar(select(ParkingSlot).where(ParkingSlot.id == booking.slot_id).with_for_update())
    if slot is None or slot.status != SlotStatus.hold:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Slot is not on hold")

    slot.status = SlotStatus.booked
    booking.hold_claimed_at = datetime.now(timezone.utc)
    booking.hold_until = None
    db.add(slot)
    db.add(booking)
    db.commit()
    db.refresh(booking)
    await broadcast_event("booking.hold_claimed", {"booking_id": str(booking.id)})
    return _serialize_booking(booking)
